...
persons-own[sexe age langue interet influence];
globals[weights]
...
to setup
  ...
  set weights matrix:make-constant nb-persons nb-persons 0

  create-persons nb-persons [
    set sexe random 2
    set age random 50
    set langue random 5
    set interet 10
...
to retweet
  ask persons[
    let index-i who
    ;;indicate if the influence dec or inc
    let influenceCP 0
    ;; does someone retweet my tweet ?
    ask persons[
      let index-j who
      if index-i != index-j [
        let random-retweet random-float  1
        let weight matrix:get weights index-j index-i
        if random-retweet < weight [
          ;;retweet ==> inlfuence[i]++
          ask persons [
            if who = index-i[
              ;;update the total influence during an episode
    ...
    let new-influence-i influence
    ;;update weights according to the influence
    ask persons[
     if who != index-i[
        show"_____________________"
        show turtle index-i
        show turtle who
        show link-with turtle index-i
        show"_____________________"
        let index-k who
        let old-weight matrix:get weights index-k index-i
        ;;following
        if old-weight != 0 [
          let random-w random-float new-influence-i
          let weight-i-k matrix:get weights index-k index-i

          ifelse random-w < 0.25 * new-influence-i [
            let new-weight-i-k-positif weight-i-k + 0.1
            matrix:set weights  index-k index-i new-weight-i-k-positif
          ]
          [
            let new-weight-i-k-negatif weight-i-k - 0.1
            ;;unfollow
            if new-weight-i-k-negatif < 0 [
              unfollow  index-k index-i
              show "unfollow"
            ]
            matrix:set weights  index-k index-i new-weight-i-k-negatif
          ]
        ]
     ]
    ]
...
to unfollow [k i]
  matrix:set weights k i 0
  ask persons [
      if who = k [
      show link-with turtle i
      ask link-with turtle i [ die ]
      ]
  ]
end
      ...
      if i != j [
        ...
        ask persons [
          if who = j[
            ...
            set j-age age
            set j-langue langue
            set j-interet interet
          ]
        ]
        let r random-float 1
        if r < proba-link [
          ;; calcul de la similarite
          let sim-age 1 - ( (abs ( i-age - j-age) ) / 50 )
          let sim-sexe 0
          if i-sexe = j-sexe [
            set sim-sexe 1
          ]
          let sim-interet 0
          if i-interet = j-interet [
            set sim-interet 1
          ]
          let sim-langue 0
          if i-langue = j-langue [
            set sim-langue 1
          ]
          let similarite 0.2 * sim-age + 0.2 * sim-sexe + 0.3 * sim-interet + 0.3 * sim-langue

          ;;to avoid symetric similarities
          let random-similarite random-float 1
          if random-similarite < proba-sim[
             matrix:set weights i j similarite
             ;; create-link-with old-node
             ask persons [
               if who = i [
                 create-link-to turtle j [ set color green ]
               ]
             ]
          ]
        ]
